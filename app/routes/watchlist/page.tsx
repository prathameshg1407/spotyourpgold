...
import { Check, Trash2, MapPin, User, IndianRupee, Star, Bed, Wifi, Car, Utensils, Shield, Zap } from "lucide-react"
...
export default function WishlistCompare() {
  ...
  const selectedListings = listings.filter((listing) => selected.includes(listing._id))
  ...
  const renderComparisonValue = (field: string, item: PGListing) => {
    switch (field) {
      case "minRent":
        return (
          <div>
            <span className="text-lg font-bold text-green-600">₹{item.minRent?.toLocaleString() || "N/A"}</span>
            <div className="text-xs text-gray-500">per month</div>
          </div>
        )
      case "roomType":
        return (
          <div className="text-sm">{item.roomType && item.roomType.length > 0 ? item.roomType.join(", ") : "N/A"}</div>
        )
      case "minSecurity":
        return (
          <div>
            <span className="font-medium">₹{item.minSecurity?.toLocaleString() || "N/A"}</span>
          </div>
        )
      case "city":
        return (
                ...
                <div className="flex justify-between items-center">
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3">
                      <h3 className="text-xl font-semibold text-gray-900">{item.pgName}</h3>
                    </div>
                ...
                <thead>
                  <tr className="border-b-2 border-gray-200">
                    ...
                    {selectedListings.map((item) => (
                      <th key={item._id} className="text-center p-4 font-bold text-gray-900 bg-gray-50 min-w-[250px]">
