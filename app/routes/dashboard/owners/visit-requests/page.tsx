...
export default function OwnerVisitRequestsPage() {
  ...
  const fetchVisitRequests = useCallback(async (page = 1) => {
    setContainerLoading("ownerVisitRequests", true);
    try {
      const response = await axios.get(`/api/owner/visit-requests?page=${page}&limit=20`);
      ...
      setContainerLoading("ownerVisitRequests", false);
    }
  }, [setContainerLoading]);
  ...
  useEffect(() => {
    filterRequests();
  }, [visitRequests, searchQuery, statusFilter, filterRequests]);
          ...
          prev.map((request) =>
            request._id === id
              ? {
                  ...request,
                  ...updates,
                  adminNotes: updates.ownerNotes || request.adminNotes,
                }
              : request
          )
        );
  ...
  const handleNotesUpdate = () => {
    if (selectedRequest) {
      updateVisitRequest(selectedRequest._id, { ownerNotes });
      setSelectedRequest({ ...selectedRequest, adminNotes: ownerNotes });
  ...
  const openNotesDialog = (request: VisitRequest) => {
    ...
    setOwnerNotes(request.adminNotes || "");
  ...
  if (containerLoading.ownerVisitRequests) {
    return (
      ...
      <div className="space-y-4">
        {filteredRequests.length === 0 ? (
          ...
          filteredRequests.map((request) => (
            <Card key={request._id} className="transition-all">
              <CardContent className="p-6">
